...
const Available = (props) => {
    ...
    const handleClickMaximumTerm = (maximum) => {
        setAvailableData({
            ...availableData,
            maximum_term: maximum
        })
    }
    return (
        <div className="zoomout">
            <h2>When is the room available, and for how long?</h2>
            <div>
            ...
            <div>
                <div className="form-check mb-2">
                    <Input
                        ...
                        name="radio5"
                        id="radio5"
                        checked={availableData.maximum_term}
                        onChange={() => handleClickMaximumTerm(true)}
                    />
                ...
                <div className="form-check">
                    <Input
                        ...
                        id="radio6"
                        checked={!availableData.maximum_term}
                        onChange={() => handleClickMaximumTerm(false)}
                    />
                    <Label className="form-check-label" for="radio6">
                        Flatmate must stay at least...
                    </Label>
                </div>
                {!availableData.maximum_term && (
                    <div>
                        <div className="input-step step-success">
                            <button
                                type="button"
                                className="minus"
                                disabled={availableData.maximum_month===1}
                                onClick={() => {
                                    setAvailableData(prevState => {
                                        return {
                                            ...prevState,
                                            maximum_month: prevState.maximum_month - 1
                                        }
                                    })
                                }}
                            >
                            ...
                            <Input
                                type="number"
                                className="product-quantity"
                                value={availableData.maximum_month}
                                min="1"
                                max="100"
                                readOnly
                            />
